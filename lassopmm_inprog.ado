...
*===============================================================================
	set seed `seed'	
	mata: mypsu = st_data(.,"`_group'", "`touse'")
	if ("`psu'"=="") noi: mata: ind = _getmyboot(mypsu,`add')		
	tempfile mydata
	save `mydata'
...
*===============================================================================
	...
	forval i=1/`add'{
		...
		if ("`psu'"==""){
			...
			if ("`addvars'"!="") mata: myvar = st_data( ., "`depvar' `_my_x' `wi' `addvars'","`touse'")
			else                 mata: myvar = st_data( ., "`depvar' `_my_x' `wi'","`touse'")
			
			homero = _getmybootpsu(mypsu)
			rr=rows
		
			info[sam[i],1],1 \ info[sam[i],2],1
			mata: myvar[,.] = myvar[,.]
...
mata
	...
	//Function for bootstrap PSU permutation vectors
	// Takes listing of PSU and samples the PSU with replacement
	// some PSU may be there more than once, or not there
	function _getmybootpsu(_psu){
		info = panelsetup(_psu,1)
		_index = runningsum(J(rows(_psu),1,1))
		rr = rows(info)
		sam= _f_sampleepsi(1,rr,(1::rr))		

		for (i=1; i<=rr; i++){					
			m = info[sam[i],1],1 \ info[sam[i],2],1
			if (i==1)        _X = _index[|m|]
			else             _X = _X \ _index[|m|]
		}
		return(_X)
